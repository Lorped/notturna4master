...
export class PersonaggioPage implements OnInit {
  ...
  constructor(public route: ActivatedRoute, public http: HttpClient) {
    ...
    var link =
      'https://www.roma-by-night.it/ionicPHP/poteri.php?id=' + this.requestID;
        ...
        .subscribe((res) => {
          ...
          for (let i = 0; i < this.myskill.length; i++) {
            ...
            if (this.myskill[i].nomeskill == 'Robustezza') {
              this.pf += this.myskill[i].livello;
              this.rp = Math.floor(
                (this.scheda['attutimento'] + this.myskill[i].livello) / 2
              );
              // vedo se ha poteri attivi

              for (let j = 0; j < this.poteri.length; j++) {
                // console.log(this.myuser.poteri[j].poteri[k]);
                if (this.poteri[j].nomepotere == 'Resilienza')
                  this.pf += 5 + this.myskill[i].livello;
                if (this.poteri[j].nomepotere == 'Sconfiggere le Debolezze')
                  this.pf += 5; //perchè +5 da liv.1
              }
            }
          ...
          this.fomipot2 = Math.ceil(
            (this.forza + this.mischia + this.potenza) / 2
          );
          this.foripot2 = Math.ceil(
            (this.forza + this.rissa + this.potenza) / 2
          );
          this.folapot2 = Math.ceil(
            (this.forza + this.lancio + this.potenza) / 2
          );
